...
The Brain layer classifies LLM output into a `BrainDecision`. The prompt asks providers to return one JSON object such as `{"kind":"say","text":"..."}`. Plain text is still accepted and treated as `say`, so normal chat remains usable even when a provider ignores the schema. The current implementation executes terminal `say` directly, allows `think` and `observe` without approval, and turns `propose_act` decisions into Human Ops approval proposals. App-launch, click, text-input, and key-press proposals all require approval; only click proposals include a red-dot preview. Every click, text-input, and key-press proposal must also name `target_app`; a proposal without a target application is not executable because Body cannot safely establish the intended macOS focus. For Human Ops desktop actions, Brain is responsible for advancing the user's target to an approvable one-step proposal: if the user asks to open an application, click or operate a visible target, type into a focused input, send with Enter, or reply in a chat surface such as WeChat, `say` is not a completion path while `goal.status` is still `in_progress`; Brain must identify the current task stage, interaction surface, usable affordance, and target application, then return the next minimal `propose_act`, which Body executes only after user approval.

Brain uses a computer-use mental model instead of a GUI-only model. It should name the current `stage`, `surface`, and `affordance` in its goal state. Surfaces can include desktop GUI, browser pages, browser chrome, terminal shells, terminal TUIs, editors, file dialogs, WeChat UI, or unknown surfaces. The current executable action set is deliberately small: `launch_app`, `click`, `type_text`, and `key_press enter`. A plain application-opening request is deterministically coerced to `launch_app` even if a provider returns the old observe/click route, so it never needs a screenshot, Dock location, screen-edge reveal, or synthetic mouse event. Human Ops shows the resolved application name and waits for approval; after approval, the desktop host reads the macOS application-bundle inventory, resolves the requested app, and starts it through Launch Services. A `click` proposal remains reviewable only when both `x` and `y` are present and parseable as macOS screen coordinates. For chat reply tasks, Brain should progress through stages such as `launch_app`, `locate_contact`, `read_context`, `draft_reply`, `focus_input`, `type_reply`, `send_reply`, and `verify_result`. In `locate_contact`, if the target contact row is not visible but a WeChat search field is visible, Brain should use the normal human path of clicking the search field, typing the contact name, and pressing Enter or clicking the search result instead of repeatedly observing the same list.
...
`screen_edge` remains available for explicit visual click tasks, but application opening does not use it. App launch failures report the requested name and bounded Launch Services error instead of falling back to screenshot-based discovery.
