...
The Brain layer classifies LLM output into a `BrainDecision`. The prompt asks providers to return one JSON object such as `{"kind":"say","text":"..."}`. Plain text is still accepted and treated as `say`, so normal chat remains usable even when a provider ignores the schema. The current implementation executes terminal `say` directly, allows `think` and `observe` without approval, and turns `propose_act` decisions into Human Ops approval proposals. App-launch, click, text-input, and key-press proposals all require approval; only click proposals include a red-dot preview. For Human Ops desktop actions, Brain is responsible for advancing the user's target to an approvable one-step proposal: if the user asks to open an application, click or operate a visible target, type into a focused input, send with Enter, or reply in a chat surface such as WeChat, `say` is not a completion path while `goal.status` is still `in_progress`; Brain must identify the current task stage, interaction surface, and usable affordance, then return the next minimal `propose_act`, which Body executes only after user approval.
