...
|-- docs/                    # architecture, workflow, ownership, optional reports, role playbooks
...
- **QA** owns regression planning and contract checks; durable reports are created only when explicitly needed.
...
- include path migration details in the requested migration record or final handoff
...
- Use `docs/reports/` only for requested or high-risk durable reports, not routine task summaries.
...
- Update `docs/WORKFLOW.md` only when a stable product contract, safety boundary, or state transition changes. Keep implementation details in code and tests.
