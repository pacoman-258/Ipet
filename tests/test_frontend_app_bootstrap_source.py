...
class FrontendAppBootstrapSourceTests(unittest.TestCase):
    ...
    def test_app_bootstrap_owns_runtime_startup_and_delegates_pet_app_api(self) -> None:
        ...
        for snippet in (
            ...
            "runtimeWindow.addEventListener(\"contextmenu\"",
            "const pointTarget = runtimeWindow.document?.elementFromPoint?.(x, y);",
            "pointTarget !== canvas || !petSceneController.isPointOnModel?.(x, y)",
            'callQtBridge("setInteractiveRegions", payload)',
            "petSceneController.modelBoundsInClientSpace?.()",
